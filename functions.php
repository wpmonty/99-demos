...
if ( ! function_exists( 'gabfire_setup_theme' ) ) {
	function gabfire_setup_theme() {
		...
		register_nav_menu( 'doc_menu', __( 'Documentation Menu', 'gabfire' ) );
		register_nav_menu( 'footer1', __( 'Footer 1', 'gabfire' ) );
		register_nav_menu( 'footer2', __( 'Footer 2', 'gabfire' ) );
		register_nav_menu( 'footer3', __( 'Footer 3', 'gabfire' ) );
...
if ( !function_exists( 'gabfire_meta_box_add' ) ) {
	...
	function gabfire_meta_box_add()
	{
		...
		add_meta_box( '', __('Product Options', 'gabfire'), 'gabfire_meta_box_doc', 'doc', 'normal', 'high' );
		add_meta_box( '', __('Product Options', 'gabfire'), 'gabfire_meta_box_product', 'download', 'normal', 'high' );
	}
	...
	function gabfire_meta_box_post( $post )
	{
		...
		$form_id = isset( $values['form_id'] ) ? esc_attr( $values['form_id'][0] ) : '';
		...
		<div class="gabfire_fieldgroup">
			...
			<p>
				...
				<select name="gabfire_post_template" id="gabfire_post_template">
					...
					<option value="rightsidebar" <?php selected( $selected, 'rightsidebar' ); ?>><?php _e('Right Sidebar','gabfire'); ?></option>
					<option value="fullwidth" <?php selected( $selected, 'fullwidth' ); ?>><?php _e('No Sidebar','gabfire'); ?></option>
					<option value="list" <?php selected( $selected, 'list' ); ?>><?php _e('Super List','gabfire'); ?></option>
		...
		<?php if ( (  get_current_post_type() == 'portfolio' ) or  ( get_current_post_type() == 'post' ) or  ( get_current_post_type() == 'page' ) ){ ?>
			...
			<div class="gabfire_fieldgroup">
				<p class="gabfire_fieldcaption"><?php _e('Gravity Form ID', 'gabfire'); ?></p>
				<p class="gabfire_fieldrow">
					<label for="form_id"><?php _e('Add the id # of the Gravity form for the landing page.','gabfire'); ?></label>
					<input type="text" class="gabfire_textinput" id="form_id" name="form_id" value="<?php echo $form_id; ?>" /></span>
				</p>
			</div>
	...
	/* Create Doc Custom Fields */
	function gabfire_meta_box_doc( $post )
	{
    	$values = get_post_custom( $post->ID );
        $selected = isset( $values['gabfire_doc_product'] ) ? esc_attr( $values['gabfire_doc_product'][0] ) : '';

		wp_nonce_field( 'my_meta_box_nonce', 'meta_box_nonce' );
		?>

		<div class="gabfire_fieldgroup">
			<p class="gabfire_fieldcaption"><?php _e('Link to Product', 'gabfire'); ?></p>
			<p>
				<label for="gabfire_doc_product"><?php _e('Select a Product to connect this doc to </br><strong>Note:</strong> Do not select if this is not a product doc.','gabfire'); ?></label>

				<select name="gabfire_doc_product" id="gabfire_doc_product">
    				...
    				?>
    					...
    					<option value="<?php echo $post->ID; ?>" <?php selected( $selected, $post->ID ); ?>><?php echo $post->post_title ?></option>
	...
	function gabfire_meta_box_product( $post )
	{
    	$values = get_post_custom( $post->ID );
        $php_version = isset( $values['gabfire_product_php_version'] ) ? esc_attr( $values['gabfire_product_php_version'][0] ) : '';

		wp_nonce_field( 'my_meta_box_nonce', 'meta_box_nonce' );
		?>

		<div>
			<p><strong><label for="gabfire_product_php_version"><?php _e('Required PHP Version','gabfire'); ?></label></strong></p>
			<p>
				<input type="text" name="gabfire_product_php_version" id="gabfire_product_php_version" value="<?php echo $php_version; ?>" />
			</p>
			<p>
				<?php _e('Enter minimum PHP version for this product to function.','gabfire'); ?>
            </p>
		</div>

    <?php } // product meta box
 ...
 ******************************************************************** */
 if ( !function_exists( 'gabfire_register_sidebar' ) ) {
	...
	function gabfire_init_widgets() {
		...
		gabfire_register_sidebar(array('name' => __('Forum', 'gabfire'),'id' => 'forum','description' => __('Forum Sidebar', 'gabfire')));
		gabfire_register_sidebar(array('name' => __('Doc', 'gabfire'),'id' => 'doc','description' => __('Documentation Sidebar', 'gabfire')));
	}
...
function gabfire_postheader() {
	...
	?>
	<section id="featured-image-holder" data-stellar-background-ratio="0.5" style="background:linear-gradient(rgba(0, 0, 0, 0.3),rgba(0, 0, 0, 0.3)), url(<?php echo $featured_image; ?>); background-position:center center; background-size:cover;">

			<div class="container pagewrap nmtop">
				<div class="row">

						<div class="title-wrapper">
							<div class="entry-title w-100">
    							<h1><?php the_title(); ?></h1>
...
// 09. ADD CUSTOM IMAGE SIZE TO THE WP MEDIA LIBRARY
// Reference:  http://wpthememakeover.com/2015/01/29/how-to-add-custom-image-sizes-to-the-wordpress-media-library/
add_image_size( 'nnr-blogpost-780', 780 );

function my_custom_sizes( $sizes ) {
  return array_merge( $sizes, array(
    'nnr-blogpost-780' => __( 'Blog Post 780px' ),
...
// 10. CHANGE EDD DEFAULT SLUG TO PRODUCTS INSTEAD OF DOWNLOADS
...
// EDD CHANGELOG SHORTCODE
/*
Use: [edd_changelog id=""]
Atts: "id" is the id of the download in EDD
*/

add_shortcode('edd_changelog','edd_changelog_sc');
...
function edd_changelog_sc($atts,$content = null) {
	...
	$out = sprintf('<pre id="edd-changelog">%s</pre>',$changelog);
